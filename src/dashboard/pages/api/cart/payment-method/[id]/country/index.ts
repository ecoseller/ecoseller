...
export const paymentMethodCountryListAPI = async (
  method: HTTPMETHOD,
  id: number,
  req?: NextApiRequest,
  res?: NextApiResponse
  ...
  const url = `/cart/dashboard/payment/method/${id}/country/`;
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (method == "GET") {
    return paymentMethodCountryListAPI("GET", Number(id), req, res)
      .then((data) => res.status(200).json(data))
      .catch((error) => res.status(400).json(null));
  } else if (method == "POST") {
    return paymentMethodCountryListAPI("POST", Number(id), req, res)
